...
CAudioUnitQueueManager::CAudioUnitQueueManager(CallbackHandler *callback, float sampleFrequency, 
											   SoundChannels channels):_callback(callback)
{
	OSStatus status;
...
OSStatus CAudioUnitQueueManager::playbackCallback(void *inRefCon, 
	...
	THIS->_callback->AudioCallbackHandler((uint8*)ioData->mBuffers[0].mData, (uint32*)&ioData->mBuffers[0].mDataByteSize);
